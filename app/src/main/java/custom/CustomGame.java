...
public class CustomGame extends AppCompatActivity implements OnClickListener, AnimationListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        (findViewById(R.id.rlNosacZvezdi)).setVisibility(View.INVISIBLE);
        ...
        this.hint.setVisibility(View.INVISIBLE);
        ...
        this.rec = getIntent().getStringExtra("ukucanarec");
        this.zadataRec = this.rec.toUpperCase();
        Typeface tf = Typeface.createFromAsset(getApplicationContext().getAssets(), "font/CarterOne.ttf");
        this.playerBrojac = this.pref.getInt("playerBrojac", 1);
        this.kategorija.setText("بازیکن " + this.playerBrojac);
        this.kategorija.setTypeface(tf, 1);
        for (i = 0; i < this.zadataRec.length(); i++) {
            this.sakrijRec += "_";
        }
        i = -1;
        while (true) {
            i = this.zadataRec.indexOf(this.razmak, i + 1);
            if (i != -1) {
    ...
    private void updateReciNaEkranu() {
        ...
        if (this.tempRastaviSlova.equals(this.rastaviSlova)) {
            ...
            this.editor.apply();
            CustomEndGamePopUp cdd = new CustomEndGamePopUp(this);
            cdd.podaci("آفرین!", "کلمه: " + this.zadataRec, this.pogresno);
            cdd.setCanceledOnTouchOutside(false);
            cdd.show();
        } else {
            if (this.zvuk > 0 && this.zvukApp) {
                this.yes.start();
            }
            this.zvuk++;
            this.tempRastaviSlova = this.rastaviSlova;
    ...
    private void azurirajSliku() {
        if (this.pogresno == 1) {
            ...
            this.editor.apply();
            CustomEndGamePopUp cdd = new CustomEndGamePopUp(this);
            cdd.podaci("اشتباهه!", "کلمه: " + this.zadataRec, this.pogresno);
            cdd.setCanceledOnTouchOutside(false);
            cdd.show();
        }
    }
